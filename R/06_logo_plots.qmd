...
logo_list_mouse_prot <- map(logo_list_mouse_prot, unique) # Filter out identical sequences
...
logo_list_human_prot <- map(logo_list_human_prot, unique) # Filter out identical sequences
...
logo_plot_mouse_prot <- ggseqlogo(logo_list_mouse_prot, ncol=7) +
  ...
  scale_x_continuous(
    breaks = 1:7,  # Existing tick positions
...
logo_comb_prot
ggsave(filename = "../results/06_logoplot_prot_fam.png", logo_comb_prot)
```
...
logo_list_mouse_enz <- map(logo_list_mouse_enz, unique) # Filter out identical sequences

logo_list_human_enz <- split(logoseq_data_human_enz$logo_seq,
                             logoseq_data_human_enz$enzyme_class)
logo_list_human_enz <- map(logo_list_human_enz, unique) # Filter out identical sequences

# Making logo plots for enzyme class
logo_plot_mouse_enz <- ggseqlogo(logo_list_mouse_enz, ncol=6) +
...
logo_plot_human_enz <- ggseqlogo(logo_list_human_enz, ncol=6) +
