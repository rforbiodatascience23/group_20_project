...
# Saving plot
ggsave(filename = "../results/05_analysis_mol_func.png",
       mol_func)
```
...
size_mass <- ggplot(data, aes(x = mass,
                 y = length,
                 color = organism)) +
...
ggsave(filename = "../results/05_analysis_size_mass.png",
       size_mass)
...
ggsave(filename = "../results/05_analysis_pH_length.png",
       pH_length)
```
...
data|>
  ...
  drop_na()|>
  distinct(entry)
# Only 5 mice in ligases which is not enough to determine anything 
```
...
ggsave(filename = "../results/05_analysis_class_size.png",
       class_size)
```
...
ggsave(filename = "../results/05_analysis_location_size.png",
       location_size)
```
