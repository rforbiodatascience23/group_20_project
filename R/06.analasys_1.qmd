...
```{r}
# Load data
data <- read_tsv("..//data/clean_data.tsv")
```

```{r}

####
# Count the number of molecular functions for each entry (belongs in augment)
####
...
merged_data <- data |>
  select(organism, entry, length, mass) |>
  distinct() |>
...
#####
merged_data |>
    ggplot(aes(x = length, y = n, fill = organism, colour = organism)) + 
