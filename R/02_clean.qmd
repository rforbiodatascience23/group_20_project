...
#| message: false
# Reading data
joined_data <- read_delim(file = "../data/01_dat_load.tsv", delim = "\t")
```
...
colnames(joined_data) <- str_replace_all(colnames(joined_data), " ", "_")
...
joined_data <- joined_data |>
  ...
  mutate(active_site = map_chr(active_site, ~paste(.x, collapse = ","))) |>
...
joined_data <- joined_data |>
  ...
  # sapply is not tidyverse 
  mutate(binding_site_pos = map_chr(binding_site_pos, ~paste(.x, collapse = ";")),
         binding_site_ligand = map_chr(binding_site_ligand, ~paste(.x, collapse = ";"))) |>
...
joined_data <- joined_data |>
  ...
  mutate(`subcellular_location_[cc]` = str_to_lower(`subcellular_location_[cc]`)) |> 
...
joined_data <- joined_data |>
  ...
  mutate(cofactor = str_to_lower(cofactor),
